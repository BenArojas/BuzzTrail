...
import { getCompletedAdventures, getNextAdventures, getPreviousAdventures } from "~/db/adventure.server"
...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  if (upcomingAdventures.length > 0) {
    const differenceInMs = upcomingAdventures[0].startDate.getTime() - today.getTime();
  ...
  return { upcomingAdventures, previousAdventures, completedAdventures, daysUntilNextAdventure, totalHikes };
}
...
export default function HomePage() {
  ...
  const { completedAdventures, daysUntilNextAdventure, previousAdventures, totalHikes, upcomingAdventures } = useLoaderData<typeof loader>();
  const progressPercentage = totalHikes === 0 ? 0 : (completedAdventures.length / totalHikes) * 100;
  ...
  return (
    <div className="container mx-auto p-6 space-y-6">
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground mt-2">
              completed {completedAdventures.length} adventures out of {totalHikes} so far!
            </p>
          </CardContent>
        </Card>
      ...
      <Card>
        ...
        <CardContent className="max-h-[calc(3*theme(spacing.16))] overflow-y-auto">
          <div className="space-y-4">
            {previousAdventures.length > 0 ? previousAdventures.map((adventure) => (
              <div key={adventure.id} className="flex items-center justify-between">
                <div>
                  <h3 className="font-semibold">{adventure.name}</h3>
                  <p className="text-sm text-muted-foreground">{adventure.startDate.toLocaleDateString('en-US', {
                    year: 'numeric',
                    month: 'long',
                    day: 'numeric',
                  })}</p>
      ...
      <Card>
        ...
        <CardContent className="max-h-[calc(3*theme(spacing.16))] overflow-y-auto">
          <div className="space-y-4">
            {upcomingAdventures.length > 0 ? upcomingAdventures.map((adventure) => (
              <div key={adventure.id} className="flex items-center justify-between">
                <div>
                  <h3 className="font-semibold">{adventure.name}</h3>
                  <p className="text-sm text-muted-foreground">{adventure.startDate.toLocaleDateString('en-US', {
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <span className="text-sm">{adventure.participants} participants</span>
                </div>
              </div>
